...
export async function getVersions(state: ICLIState) {
    const versions = await migrateLib.versions.getVersions(state.paths.migrations);
    if (!versions.length) {
...
export async function migrate(state: ICLIState, versions: string[], targetVersion: string) {
    ...
    await migrateLib.project.migrateProject(state.project, targetVersion, migrateThroughVersions);
}
