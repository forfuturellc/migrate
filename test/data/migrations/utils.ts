...
export async function setCurrentVersion(version: string) {
    await new Promise(function(resolve, reject) {
        return fs.writeFile(commonPaths.currentVersion, version, function(
            error,
        ) {
            if (error) {
                return reject(error);
...
export async function recordCompletion() {
    await new Promise(function(resolve, reject) {
        return fs.writeFile(commonPaths.completion, "1", (e) =>
            e ? reject(e) : resolve(),
        );
    });
}
export async function isCompleted() {
    return await new Promise(function(resolve) {
        return fs.stat(commonPaths.completion, (e) =>
            e ? resolve(false) : resolve(true),
        );
    });
}

export async function recordMigratedVersion(
    direction: "up" | "down",
    version: string,
) {
    const migratedVersions = (await new Promise(function(resolve, reject) {
        return fs.readFile(commonPaths.migratedVersions, "utf8", function(
            error,
            contents,
        ) {
    ...
    await new Promise(function(resolve, reject) {
        return fs.writeFile(
            commonPaths.migratedVersions,
            JSON.stringify(migratedVersions),
            function(error) {
                if (error) {
                    return reject(error);
                }
